<template>
  <div class="dashboard">
    <PermanentFull :titre="titre" :icon="icon" :menuItems="menuItems">
      ...
      >
        <div id="details-support" class="mx-0">
          <div class="d-flex flex-column justify-content-center">
            <router-link to="/support" class="back mb-3 align-self-start"
              ><i class="fa fa-chevron-left" aria-hidden="true"></i>
              {{ $t("back") }}</router-link
            >
            <div class="titre-recherche">{{ $t("detail:Ticket") }}</div>
            <p class="description">
              {{ $t("intro:ticketDetail") }}
            </p>
          </div>
        </div>
        <div
        ...
        >
          <div class="titre-wrapper">
            <p>{{ ticketName }}</p>
            <div
              class="statut d-flex align-items-center"
          ...
          <div class="message-wrapper p-2 p-md-3" id="messageWrapper">
            <div class="message-list">
              ...
              >
                ...
                <div
                  ...
                  :key="index"
                  :class="item.is_admin ? 'sender-wrapper' : 'receiver-wrapper'"
                >
                  <div>
          ...
          <!-- add new message -->
          <div class="form-wrapper d-flex p-2 p-md-3">
...
export default {
  ...
  methods: {
    ...
    getTicketById: async function() {
      await this.$store.dispatch("getTicketById", {
        id: this.userTicket,
  ...
  created() {
    this.userTicket = +this.decryptValue(localStorage.getItem("supportId"));
  },
  computed: {
    ticket() {
      return this.$store.state.ticket.tickets.ticket;
    },
    messages() {
      return _.groupBy(
        this.$store.state.ticket.tickets.messages,
        "message_date"
      );
    },
    user() {
...
<style>
#content-dashboard-wrapper .messagebox .message-wrapper {
  /* background-color: green; */
  flex: 1 1 100%;
  height: calc(100% - 170px);
}
#content-dashboard-wrapper .messagebox .message-list {
  display: grid;
  align-items: end;
  height: 100%;
  max-height: 100%;
...
/* .messagebox .new_message_wrapper .close {
    border-radius: 6px;
